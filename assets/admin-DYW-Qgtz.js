import{a as e,i as t,n,o as r,r as i,t as a}from"./config-DyUWov6t.js";r((async()=>{await i();var r=document.getElementById(`authenticating`),o=document.getElementById(`mainContainer`),s=document.getElementById(`userForm`),c=document.getElementById(`submitBtn`),l=document.getElementById(`btnText`),u=document.getElementById(`btnLoader`),d=document.getElementById(`statusMessage`),f=document.getElementById(`displayArea`),p=document.getElementById(`user`),m=document.getElementById(`type`),h=document.getElementById(`distanceContainer`),g=document.getElementById(`distance`),_=document.getElementById(`loadDataBtn`),v=document.getElementById(`logoutBtn`),y=document.getElementById(`adminName`),b=document.getElementById(`timeInputContainer`),x=document.getElementById(`happyBirthday`),S=document.getElementById(`userPage`);S.onclick=()=>{window.open(n(),`_blank`)};var C=new Date().toLocaleDateString(`ja-JP`,{year:`numeric`,month:`2-digit`,day:`2-digit`}).split(`/`).join(`-`);document.getElementById(`date`).value=C;var w=[`00:05`,`00:10`,`00:15`,`00:30`,`00:45`,`01:00`,`01:15`,`01:30`,`01:45`,`02:00`,`02:15`,`02:30`,`02:45`,`03:00`];async function T(){if(!(void 0).includes(`YOUR_WEB_APP_URL`)){f.innerHTML=t(`Data Loading .....`);try{O((await(await fetch(void 0)).json()).sort((e,t)=>{let n=e[3],r=t[3];return n.localeCompare(r)}))}catch(e){console.error(`Fetch error:`,e),f.innerHTML=`<p class="text-red-500 text-center py-10 text-sm">Error connecting to database.</p>`}}}_.addEventListener(`click`,T);var E=async()=>{let t=await e(`admin`);t&&t.status===`success`&&(y.textContent=t.name,p.value=t.user,t.birthdayInfo.today&&j(t.birthdayInfo),r.classList.add(`hidden`),o.classList.remove(`hidden`),T())};document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,E):E();function D(){let e=m.value;[`Walking`,`Running`].includes(e)?(h.classList.remove(`hidden`),g.required=!0,b.innerHTML=`
        ...
        `)}m.addEventListener(`change`,D),D(),s.addEventListener(`submit`,async function(e){if(e.preventDefault(),(void 0).includes(`YOUR_WEB_APP_URL`)){A(`Please provide a valid Web App URL.`,`error`);return}k(!0);let t={action:`addActivity`,user:String(p.value),type:String(document.getElementById(`type`).value),date:`'`+String(document.getElementById(`date`).value),time:`'`+String(document.getElementById(`time`).value),distance:g.value?String(g.value):`-`,admin:y.textContent,userIp:a.ipaddress};try{await fetch(void 0,{method:`POST`,mode:`no-cors`,cache:`no-cache`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t)}),A(`Activity successfully recorded!`,`success`);let e=p.value;s.reset(),p.value=e,document.getElementById(`date`).value=C,D(),setTimeout(T,1e3)}catch(e){console.error(`Error:`,e),A(`Network error. Check console.`,`error`)}finally{k(!1)}});function O(e){let t=e.filter(e=>e&&e.length>=5&&e[1]!==`User`);if(t.length===0){f.innerHTML=`<p class="text-slate-400 text-center py-10 text-sm">The history is currently empty.</p>`;return}f.innerHTML=[...t].reverse().map(e=>{let t=e[0],n=e[1],r=e[2],i=e[3],a=e[4],o=e[5],s=[`Walking`,`Running`].includes(r);return`
